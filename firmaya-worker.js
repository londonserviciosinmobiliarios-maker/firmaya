  ...
  <div style="background:white;border-radius:16px;padding:32px">
    ...
    if(url.pathname === '/api/signed-pdf' && request.method === 'POST'){
      try{
        ...
        const firmante = formData.get('firmante') || '';
        const docNombre= formData.get('docNombre')|| 'documento-firmado.pdf';

        if(!email || !pdfBlob) return json({ok:false, error:'Faltan datos'}, 400);
        if(!env.RESEND_API_KEY) return json({ok:false, error:'Sin clave Resend'}, 500);
        ...
        const docTitle = docNombre.replace(/-firmado\.pdf$/i,'').replace(/\.pdf$/i,'');
        ...
        return json({ ok: emailResp.ok, emailId: emailData.id });
    ...
    if(url.pathname === '/api/acm-save' && request.method === 'POST'){
      try{
        const body = await request.json();
        const { slug, agente, nombre, barrio, tipo, valorFinal, fecha, data } = body;
        if(!slug || !agente) return json({ok:false, error:'Faltan datos'}, 400);
        if(!env.FIRMAYA_KV) return json({ok:false, error:'KV no configurado'}, 500);
        const key = 'acm:' + agente.toLowerCase().replace(/\s+/g,'-') + ':' + slug;
        await env.FIRMAYA_KV.put(key, JSON.stringify({
          slug, agente, nombre, barrio, tipo, valorFinal, fecha,
    ...
    if(url.pathname === '/api/acm-list'){
      if(!env.FIRMAYA_KV) return json({ok:false, error:'KV no configurado'}, 500);
      try{
        const agente = url.searchParams.get('agente'); // opcional: filtrar por agente
        const prefix = agente ? 'acm:' + agente.toLowerCase().replace(/\s+/g,'-') + ':' : 'acm:';
        const list = await env.FIRMAYA_KV.list({ prefix });
        const acms = await Promise.all(list.keys.map(k => env.FIRMAYA_KV.get(k.name, 'json')));
        const sorted = acms.filter(Boolean).sort((a,b) => {
          const ta = a.guardadoEn ? new Date(a.guardadoEn).getTime() : 0;
          const tb = b.guardadoEn ? new Date(b.guardadoEn).getTime() : 0;
          return tb - ta;
        });
        return json({ ok: true, acms: sorted });
      }catch(e){ return json({ ok: false, error: e.message }, 500); }
    }
    ...
    if(url.pathname === '/api/acm-load'){
      ...
      try{
        const key = 'acm:' + agente.toLowerCase().replace(/\s+/g,'-') + ':' + slug;
        const data = await env.FIRMAYA_KV.get(key, 'json');
        if(!data) return json({ok:false, error:'ACM no encontrado'}, 404);
        return json({ ok: true, acm: data });
      }catch(e){ return json({ ok: false, error: e.message }, 500); }
    }
    ...
    if(url.pathname === '/api/acm-delete' && request.method === 'POST'){
      try{
        const body = await request.json();
        const { slug, agente } = body;
        if(!slug || !agente) return json({ok:false, error:'Faltan datos'}, 400);
        if(!env.FIRMAYA_KV) return json({ok:false, error:'KV no configurado'}, 500);
        const key = 'acm:' + agente.toLowerCase().replace(/\s+/g,'-') + ':' + slug;
        await env.FIRMAYA_KV.delete(key);
        return json({ ok: true });
      }catch(e){ return json({ ok: false, error: e.message }, 500); }
    }
